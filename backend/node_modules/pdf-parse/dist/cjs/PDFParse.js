...
const ImageResult_js_1 = require("./ImageResult.js");
const PathGeometry_js_1 = require("./PathGeometry.js");
const TextResult_js_1 = require("./TextResult.js");
...
class PDFParse {
    ...
    async GetText() {
        const result = { ...TextResult_js_1.TextResultDefault };
    ...
    async GetImage() {
        const result = { ...ImageResult_js_1.ImageResultDefault };
        const infoData = await this.load();
        Object.assign(result, infoData);
        if (this.doc === undefined) {
            throw new Error('PDF document not loaded');
            ...
            if (fn === pdfjs.OPS.constructPath) {
                while (args[0].length) {
                    ...
                    if (op === pdfjs.OPS.rectangle) {
                        ...
                        const rect = new Geometry_js_1.Rectangle(new Geometry_js_1.Point(x, y), width, height);
                        ...
                        const y = args[1].shift();
                        //default trasform
                        const from = new Geometry_js_1.Point(current_x, current_y);
                        const to = new Geometry_js_1.Point(x, y);
                        const line = new Geometry_js_1.Line(from, to);
